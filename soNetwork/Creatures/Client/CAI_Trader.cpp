...
//#define CAI_TRADER_DEBUG

#include "../../../shared/hook.h"

#ifdef CAI_TRADER_DEBUG
...
void CAI_Trader__NET_Export(DWORD pClass,NET_Packet* pack)
{
	...
	LogHandle->Write("CAI_Trader::NET_Export(0x%X,0x%X)",pClass,pack);
	LogHandle->Write("{");
...
void CAI_Trader__NET_Import(DWORD pClass,NET_Packet* pack)
{
	...
	LogHandle->Write("CAI_Trader::NET_Import(0x%X,0x%X)",pClass,pack);
	LogHandle->Write("{");
