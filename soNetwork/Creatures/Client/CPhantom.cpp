...
//#define CPHANTOM_DEBUG

#include "../../../shared/hook.h"

#ifdef CPHANTOM_DEBUG
...
void CPhantom__NET_Export(DWORD pClass,NET_Packet* pack)
{
	...
	LogHandle->Write("CPhantom::NET_Export(0x%X,0x%X)",pClass,pack);
	LogHandle->Write("{");
...
void CPhantom__NET_Import(DWORD pClass,NET_Packet* pack)
{
	...
	LogHandle->Write("CPhantom::NET_Import(0x%X,0x%X)",pClass,pack);
	LogHandle->Write("{");
