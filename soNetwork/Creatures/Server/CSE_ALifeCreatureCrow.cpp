...
//#define CSE_ALIFE_CREATURE_CROW_DEBUG

#include "../../../shared/hook.h"

#ifdef CSE_ALIFE_CREATURE_CROW_DEBUG
	#include "../../../shared/log.h"
	extern CLog* LogHandle;
#endif

struct CSE_ALifeCreatureCrow_sync_buffer
{
	FLOAT	unknown[3];
	FLOAT	unknown2[3];
	FLOAT	direction[3];
...
extern DWORD	pCSE_ALifeCreatureCrow__UPDATE_Write;
extern DWORD	pCSE_ALifeCreatureCrow__UPDATE_Read;
...
void CSE_ALifeCreatureCrow__UPDATE_Write(DWORD pClass,NET_Packet* pack)
{
	PUSH_REG(ecx)
	...
	if(orig_reg != pUPDATE_Write_call_address) // called not from xrServer::SyncAllCreautres
	{
		_asm
		{
			push pack
			mov ecx, pClass
			call pCSE_ALifeCreatureCrow__UPDATE_Write
		}
	...
	CSE_ALifeCreatureCrow_sync_buffer* sync_buffer = (CSE_ALifeCreatureCrow_sync_buffer*)CustomSyncData_Entity[objectid];
	...
	LogHandle->Write("CSE_ALifeCreatureCrow::UPDATE_Write(0x%X,0x%X)",pClass,pack);
	LogHandle->Write("{");
...
void CSE_ALifeCreatureCrow__UPDATE_Read(DWORD pClass,NET_Packet* pack)
{
	PUSH_REG(ecx)
	...
	if(orig_reg != pUPDATE_Read_call_address) // called not from xrServer::Process_update
	{
		_asm
		{
			push pack
			mov ecx, pClass
			call pCSE_ALifeCreatureCrow__UPDATE_Read
		}
	...
	if(!sync_buffer) 
	{
		sync_buffer = new CSE_ALifeCreatureCrow_sync_buffer;
	...
	LogHandle->Write("CSE_ALifeCreatureCrow::UPDATE_Read(0x%X,0x%X)",pClass,pack);
	LogHandle->Write("{");
