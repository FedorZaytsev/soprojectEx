...
//#define CSE_ALIFE_CREATURE_PHANTOM_DEBUG

#include "../../../shared/hook.h"

#ifdef CSE_ALIFE_CREATURE_PHANTOM_DEBUG
	#include "../../../shared/log.h"
	extern CLog* LogHandle;
#endif

struct CSE_ALifeCreaturePhantom_sync_buffer
{
	FLOAT	unknown[3];
	FLOAT	unknown2[3];
	FLOAT	direction[3];
...
extern DWORD	pCSE_ALifeCreaturePhantom__UPDATE_Write;
extern DWORD	pCSE_ALifeCreaturePhantom__UPDATE_Read;
...
void CSE_ALifeCreaturePhantom__UPDATE_Write(DWORD pClass,NET_Packet* pack)
{
	...
	if(orig_reg != pUPDATE_Write_call_address) // called not from xrServer::SyncAllCreautres
	{
		_asm
		{
			push pack
			mov ecx, pClass
			call pCSE_ALifeCreaturePhantom__UPDATE_Write
		}
	...
	CSE_ALifeCreaturePhantom_sync_buffer* sync_buffer = (CSE_ALifeCreaturePhantom_sync_buffer*)CustomSyncData_Entity[objectid];
	...
	LogHandle->Write("CSE_ALifeCreaturePhantom::UPDATE_Write(0x%X,0x%X)",pClass,pack);
	LogHandle->Write("{");
...
void CSE_ALifeCreaturePhantom__UPDATE_Read(DWORD pClass,NET_Packet* pack)
{
	...
	if(orig_reg != pUPDATE_Read_call_address) // called not from xrServer::Process_update
	{
		_asm
		{
			push pack
			mov ecx, pClass
			call pCSE_ALifeCreaturePhantom__UPDATE_Read
		}
	...
	if(!sync_buffer) 
	{
		sync_buffer = new CSE_ALifeCreaturePhantom_sync_buffer;
	...
	LogHandle->Write("CSE_ALifeCreaturePhantom::UPDATE_Read(0x%X,0x%X)",pClass,pack);
	LogHandle->Write("{");
